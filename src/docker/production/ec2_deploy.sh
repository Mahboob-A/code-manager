...
tar --exclude='.git' --exclude='./venv' --exclude='/__pycache__/' -cvf ./production_project.tar .


echo "Uploading the Project to the server ... " 

rsync -e "ssh -i ${GENP_ALGOCM_EC2_PEM}" ./production_project.tar ubuntu@"${GENP_ALGOCM_EC2_IP_ADDR}":/tmp/production_project.tar
...
ssh -i "${GENP_ALGOCM_EC2_PEM}" -o StrictHostKeyChecking=no ubuntu@"${GENP_ALGOCM_EC2_IP_ADDR}" << 'ENDSSH'
    ...
    PROJECT_PATH=/home/ubuntu/algocode-cm-service/backend 
    LOG_PATH=/home/ubuntu/algocode-cm-service/logs
    WAIT_TIME=10
    ...
    docker_compose_down_output=$(docker compose -p code_manager -f "$PROJECT_PATH"/production.yml down)
    ...
    docker_compose_up_output=$(sudo docker compose -p code_manager -f $PROJECT_PATH/production.yml up --build -d --remove-orphans)
