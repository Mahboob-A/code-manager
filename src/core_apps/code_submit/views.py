...
# local
from core_apps.code_submit.utils import decode_jwt, process_data, UploadToS3
...
class SubmitCode(APIView):
    ...
    def post(self, request, format=None):
        ...
        data, message = process_data(request=request, problem_id=problem_id)
        if data is not None:
            now = int(time.time())
            ...
            # Upload to S3 
            file_url, message = UploadToS3.upload_file(
            ...
            logger.error(f"\nData could not be processed. The error is: {message}")
